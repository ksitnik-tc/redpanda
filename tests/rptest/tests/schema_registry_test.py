...
class SchemaRegistryAutoAuthTest(SchemaRegistryTestMethods):
    ...
    No RPC transport variant: auto-auth uses ephemeral credentials that
    require the Kafka client transport for SASL credential propagation.
    """
...
class SchemaRegistryTransportStressTest(SchemaRegistryEndpoints):
    ...
    def test_no_errors_during_leadership_transfers(self):
        ...
        # Spot-check that the system recovers after transfers complete.
        # Use a short timeout — on overloaded CI boxes the SR may be
        # slow to respond, but that's environmental, not a code bug.
        for node in self.redpanda.nodes:
            hostname = node.account.hostname
            try:
                r = self.sr_client.get_subjects(hostname=hostname, timeout=req_timeout)
                ...
                self.logger.warn(
                    f"Post-transfer GET /subjects on {hostname} failed: {e}"
                )
