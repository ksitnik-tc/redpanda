...
std::string_view to_string_view(request_state e) {
    switch (e) {
...
fmt::iterator producer_state::format_to(fmt::iterator it) const {
    ...
    if (_transaction_state) {
        return fmt::format_to(it, "transaction_state: {}", *_transaction_state);
    } else {
        return fmt::format_to(it, "transaction_state: {{ null }}");
    }
