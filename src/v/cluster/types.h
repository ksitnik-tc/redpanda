...
inline std::string_view to_string_view(incremental_update_operation op) {
    return incremental_update_operation_as_string(op);
}
...
std::string_view to_string_view(topic_purge_domain);
...
std::string_view to_string_view(reconfiguration_policy);
...
std::string_view to_string_view(partition_operation_type);
...
std::string_view to_string_view(topic_table_topic_delta_type);
...
std::string_view to_string_view(topic_table_ntp_delta_type);
...
std::string_view to_string_view(recovery_stage s);
...
std::string_view to_string_view(reconciliation_status);
...
std::string_view to_string_view(partition_move_direction);
...
std::string_view to_string_view(node_update_type);
...
std::string_view to_string_view(reconfiguration_state);
...
std::string_view to_string_view(cloud_storage_mode);
...
template<>
struct fmt::formatter<cluster::backend_operation> {
    constexpr auto parse(fmt::format_parse_context& ctx) { return ctx.begin(); }
    auto format(
      const cluster::backend_operation& v, fmt::format_context& ctx) const {
