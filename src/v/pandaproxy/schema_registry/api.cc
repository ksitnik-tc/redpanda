    ...
    if (use_rpc) {
        ...
        co_await _client.start(
          ...
          [this](std::exception_ptr ex) {
              // During _client.start(), the transport variant is still
              // monostate — just rethrow. Once the transport is
              // emplaced, delegate to its mitigate_error().
              return ss::visit(
                _transport,
                [eptr = ex](std::monostate) {
                    return ss::make_exception_future<>(eptr);
                },
                [eptr = ex](auto& t) {
                    return t.local().mitigate_error(eptr);
                });
          });
...
ss::future<> api::stop() {
    ...
    const bool kafka_mode
      = std::holds_alternative<ss::sharded<kafka_client_transport>>(_transport);
    if (kafka_mode) {
